...
void usb_isr(const usb_dev id)
{
    ...
    if (intr_tx & BIT(0)) {             // endp0 in out setup event 
        ...
        for (int i = 1; i < USBD_ENDP_NUM; i++) {
            if (intr_rx & BIT(i)) {
                // uint8_t buf[64];
                // uint32_t len = usb_g_bulk_read(id, i, buf, 31, 1);
                // logd("usbd_ep_buf=%x\n",(uint32_t)usbd_ep_buf);dumpd(usbd_ep_buf, 64);
                // logd("len=%d:",len);dumpd(buf, len);
...
error_t hal_usbd_init(uint8_t id)
{
    ...
    usb_slave_init(id);
    usb_set_dma_raddr(id, 0, ep0_dma_buffer);
    usb_set_dma_raddr(id, 1, ep0_dma_buffer);       //TODO
